...
def bit_move(v: int, c: int):
    return ((v << c) & 0xffffffff) | v >> (32 - c)


def quarter_round(x: list, a: int, b: int, c: int, d: int):
    x[a] = (x[a] + x[b]) & 0xffffffff
    x[d] = bit_move(x[d] ^ x[a], 16)
    x[c] = (x[c] + x[d]) & 0xffffffff
    x[b] = bit_move(x[b] ^ x[c], 12)
    x[a] = (x[a] + x[b]) & 0xffffffff
    x[d] = bit_move(x[d] ^ x[a], 8)
    x[c] = (x[c] + x[d]) & 0xffffffff
    x[b] = bit_move(x[b] ^ x[c], 7)


def diagonal_round(y: list):
    quarter_round(y, 0, 5, 10, 15)
    quarter_round(y, 1, 6, 11, 12)
    quarter_round(y, 2, 7, 8, 13)
    quarter_round(y, 3, 4, 9, 14)


def column_round(y: list):
    quarter_round(y, 0, 4, 8, 12)
    quarter_round(y, 1, 5, 9, 13)
    quarter_round(y, 2, 6, 10, 14)
    quarter_round(y, 3, 7, 11, 15)


def yield_chacha(key, iv, position=0):
    ...
    ctx = [0] * 16
    ctx[:4] = (0x61707865, 0x3320646e, 0x79622d32, 0x6b206574)
    # print(struct.unpack('<8L', key), key, "key")
...
def chacha20_encrypt(data, key, iv=None, position=0):
    ...
    return bytes(a ^ b for a, b in zip(data, yield_chacha(key, iv, position)))
